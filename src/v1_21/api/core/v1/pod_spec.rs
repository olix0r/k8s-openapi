            ...
            fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error> where A: serde::de::MapAccess<'de> {
                ...
                while let Some(key) = serde::de::MapAccess::next_key::<Field>(&mut map)? {
                    match key {
                        ...
                        Field::Key_set_hostname_as_fqdn => value_set_hostname_as_fqdn = serde::de::MapAccess::next_value(&mut map)?,
...
impl serde::Serialize for PodSpec {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
        ...
        if let Some(value) = &self.set_hostname_as_fqdn {
            serde::ser::SerializeStruct::serialize_field(&mut state, "setHostnameAsFQDN", value)?;
        }
