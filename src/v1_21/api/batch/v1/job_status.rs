            ...
            fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error> where A: serde::de::MapAccess<'de> {
                ...
                while let Some(key) = serde::de::MapAccess::next_key::<Field>(&mut map)? {
                    match key {
                        ...
                        Field::Key_completed_indexes => value_completed_indexes = serde::de::MapAccess::next_value(&mut map)?,
...
impl serde::Serialize for JobStatus {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
        ...
        if let Some(value) = &self.completed_indexes {
            serde::ser::SerializeStruct::serialize_field(&mut state, "completedIndexes", value)?;
        }
